...
| 1.1.2 | Vérifiez que les processus de formation des données excluent les caractéristiques, attributs ou champs inutiles (par exemple, les métadonnées non utilisées, les informations personnelles sensibles, les données de test divulguées). |   1    | D/V  |
| 1.1.3 | Vérifiez que toutes les modifications des ensembles de données sont soumises à un processus d'approbation consigné.                                                                                                                    |   2    | D/V  |
| 1.1.4 | Vérifiez que les ensembles de données ou sous-ensembles sont marqués par un filigrane ou estampillés où cela est faisable.                                                                                                             |   3    | D/V  |
...
| 1.2.2 | Vérifiez que tout accès aux données d'entraînement est consigné, y compris l'utilisateur, l'heure et l'action.                                                                                                 |   2    | D/V  |
| 1.2.3 | Vérifiez que les ensembles de données d'entraînement sont chiffrés en transit et au repos, en utilisant des algorithmes cryptographiques standard de l'industrie et des pratiques de gestion des clés.         |   2    | D/V  |
| 1.2.4 | Vérifiez que des fonctions de hachage cryptographique ou des signatures numériques sont utilisées pour garantir l'intégrité des données lors du stockage et du transfert des données d'entraînement.           |   2    | D/V  |
| 1.2.5 | Vérifiez que des techniques de détection automatisées sont appliquées pour prévenir les modifications non autorisées ou la corruption des données d'entraînement.                                              |   2    | D/V  |
| 1.2.6 | Vérifiez que les données d'entraînement obsolètes sont supprimées de manière sécurisée ou anonymisées.                                                                                                         |   2    | D/V  |
| 1.2.7 | Vérifiez que toutes les versions du jeu de données d'entraînement sont identifiées de manière unique, stockées de façon immuable et auditées pour permettre un retour en arrière et une analyse médico-légale. |   3    | D/V  |
...
|   #   | Description                                                                                                                                                                                                                                                     | Niveau | Rôle |
| :---: | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :----: | :--: |
| 1.3.1 | Vérifiez que des hachages cryptographiques ou des signatures numériques sont appliqués aux artefacts d'étiquette afin d'assurer leur intégrité et leur authenticité.                                                                                            |   2    | D/V  |
| 1.3.2 | Vérifiez que les interfaces et plateformes d'étiquetage appliquent des contrôles d'accès stricts, maintiennent des journaux d'audit résistants à la falsification pour toutes les activités d'étiquetage, et protègent contre les modifications non autorisées. |   2    | D/V  |
| 1.3.3 | Vérifiez que les informations sensibles dans les étiquettes sont redigées, anonymisées ou cryptées au niveau du champ de données, au repos et en transit.                                                                                                       |   3    | D/V  |
...
| 1.4.4 | Vérifiez que les labels générés automatiquement (par exemple, via des LLM ou une supervision faible) sont soumis à des seuils de confiance et à des vérifications de cohérence afin de détecter les labels hallucinatifs, trompeurs ou à faible confiance.                                                                                                                                                                                                                                                                               |   2    | D/V  |
| 1.4.5 | Vérifiez que les tests automatisés détectent les décalages d'étiquettes à chaque ingestion ou transformation significative des données.                                                                                                                                                                                                                                                                                                                                                                                                  |   3    |  D   |
