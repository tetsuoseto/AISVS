...
 Ensure that all versions of the training dataset are uniquely identified, stored immutably, and are auditable to enable rollback and forensic analysis.
...
 Verify that labeling interfaces and platforms enforce robust access controls, maintain tamper-evident audit logs for all labeling activities, and protect against unauthorized modifications.
 #1.3.3    Level: 3    Role: D/V
 Verify that sensitive information in labels is redacted, anonymized, or encrypted at the data field level both at rest and during transit.
...
 Verify that automated tests catch format errors and nulls during every data ingestion or significant transformation.
 #1.4.2    Level: 2    Role: D/V
 Ensure that LLM training and fine-tuning pipelines include poisoning detection and data integrity validation (e.g., statistical methods, outlier detection, embedding analysis) to identify potential poisoning attacks (e.g., label flipping, backdoor trigger insertion, role-switching commands, influential instance attacks) or unintentional data corruption in the training data.
...
 Verify that the lineage of each data point, including all transformations, augmentations, and merges, is documented and can be fully reconstructed.
...
 Ensure that adversarial evaluation tests (e.g., Red Team "many-shot" prompts) are conducted before every model or prompt-template release, with success-rate thresholds and automated blockers to prevent regressions.
 #2.1.4    Level: 2    Role: D
 Ensure that prompts coming from third-party content (webpages, PDFs, emails) are sanitized within an isolated parsing context before being concatenated into the main prompt.
...
Natural Language Processing (NLP) models remain vulnerable to subtle character- or word-level perturbations that humans often overlook, but that models tend to misclassify.

 #2.2.1    Level: 1    Role: D
 Ensure that basic input normalization steps—such as Unicode NFC normalization, homoglyph mapping, and whitespace trimming—are performed before tokenization.
 #2.2.2    Level: 2    Role: D/V
 Verify that statistical anomaly detection flags inputs with unusually high edit distances to language norms, excessive repeated tokens, or abnormal embedding distances.
...
 Ensure that suspected adversarial inputs are quarantined and logged with full payloads (after PII redaction).
 #2.2.5    Level: 3    Role: D/V
 Ensure that robustness metrics (success rates of known attack suites) are monitored over time and that any regressions trigger a release blocker.
...
 Ensure that type checks (e.g., numeric ranges, enum values, MIME types for images/audio) are enforced on the server side, not just in the client code.
...
Developers should be able to detect syntactically valid prompts that request disallowed content (such as illicit instructions, hate speech, or copyrighted text) and then prevent them from spreading.
...
 Verify that inputs violating policies receive standardized refusals or safe completions to prevent their propagation to downstream LLM calls.
...
 Ensure that screening complies with user-specific policies (such as age and regional legal restrictions) through attribute-based rules that are evaluated at the time of the request.
...
 Verify that abuse prevention logs are retained and reviewed to identify emerging attack patterns.

---

### C2.6 Multi-Modal Input Validation

AI systems should incorporate strong validation for non-text inputs (such as images, audio, and files) to prevent injection attacks, evasion, or resource misuse.
...
 Verify that failures in multi-modal input validation are logged and trigger alerts for investigation.

---

### C2.7 Input Provenance and Attribution

AI systems should facilitate auditing, abuse tracking, and compliance by monitoring and tagging the sources of all user inputs.

 #2.7.1    Level: 1    Role: D/V
 Ensure that all user inputs are tagged with metadata (user ID, session, source, timestamp, IP address) during ingestion.
...
 Verify that adaptive threat detection incorporates machine learning models that adjust threat sensitivity according to attack frequency and success rates.
...
Developers should implement security validation for text, image, audio, and other AI input modalities using specific types of threat detection and resource isolation.

 #2.9.1    Level: 1    Role: D/V
 Ensure that each input modality has dedicated security validators with documented threat patterns (text: prompt injection, images: steganography, audio: spectrogram attacks) and defined detection thresholds.
...
 Verify that cross-modal attack detection identifies coordinated attacks involving multiple input types (e.g., steganographic payloads in images combined with prompt injection in text) using correlation rules and alert generation.
 #2.9.4    Level: 3    Role: D/V
 Verify that multi-modal validation failures trigger detailed logging that includes all input modalities, validation results, threat scores, and correlation analysis using structured log formats for SIEM integration.
...
 Ensure that all model artifacts (weights, configurations, tokenizers) are cryptographically signed by authorized entities prior to deployment.
 #3.1.2    Level: 1    Role: D/V
 Verify that model integrity is validated at deployment and that signature verification failures prevent the model from loading.
 #3.1.3    Level: 2    Role: D/V
 Verify that model provenance records include the authorizing entity’s identity, training data checksums, validation test results indicating pass/fail status, and a creation timestamp.
 #3.1.4    Level: 2    Role: D/V
 Ensure that all model artifacts use semantic versioning (MAJOR.MINOR.PATCH) with clearly documented criteria defining when each version component should be incremented.
 #3.1.5    Level: 2    Role: V
 Verify that dependency tracking maintains a real-time inventory that allows for the rapid identification of all consuming systems.
...
 Ensure that models undergo automated security testing, which includes input validation, output sanitization, and safety evaluations, with pre-agreed organizational pass/fail thresholds before deployment.
 #3.2.2    Level: 1    Role: D/V
 Verify that validation failures automatically block model deployment unless explicitly overridden with approval from pre-designated authorized personnel who provide documented business justifications.
...
 Ensure that production deployments use gradual rollout mechanisms (such as canary deployments and blue-green deployments) with automated rollback triggers based on predefined error rates, latency thresholds, or security alert criteria.
 #3.3.2    Level: 1    Role: D/V
 Ensure that rollback capabilities restore the entire model state (weights, configurations, dependencies) atomically within predefined organizational time windows.
 #3.3.3    Level: 2    Role: D/V
 Ensure that deployment processes validate cryptographic signatures and compute integrity checksums before activating the model, and fail the deployment if any mismatches occur.
...
 Ensure that rollback artifacts (previous model versions, configurations, dependencies) are preserved according to organizational policies using immutable storage for incident response.
...
 Verify that all model changes (deployment, configuration, retirement) generate immutable audit records that include a timestamp, an authenticated actor identity, a change type, and the before and after states.
...
 Ensure that prompt templates and system messages are version-controlled in Git repositories, with mandatory code review and approval from designated reviewers prior to deployment.
...
 Verify that model development, testing, and production environments are physically or logically separated. They should have no shared infrastructure, distinct access controls, and isolated data storage.
...
 Verify that model development artifacts (hyperparameters, training scripts, configuration files) are stored in version control and that peer review approval is required before using them in training.
...
 Ensure that model retirement events are logged with timestamps and actor identities, and that model signatures are revoked to prevent reuse.
...
 Verify that seccomp profiles block all syscalls except those on pre-approved allowlists, with any violations terminating the container and triggering security alerts.
...
 Verify that eBPF-based runtime monitoring (such as Falco, Tetragon, or an equivalent solution) detects privilege escalation attempts and automatically terminates offending processes within the organization's response time requirements.
...
 Ensure that infrastructure-as-code is scanned with tools (tfsec, Checkov, or Terrascan) on every commit, blocking merges that contain CRITICAL or HIGH severity findings.
 #4.2.2    Level: 1    Role: D/V
 Verify that container builds are reproducible with identical SHA256 hashes across multiple builds and generate SLSA Level 3 provenance attestations signed using Sigstore.
 #4.2.3    Level: 2    Role: D/V
 Ensure that container images include embedded CycloneDX or SPDX SBOMs and are signed with Cosign before pushing to the registry, with unsigned images being rejected during deployment.
 #4.2.4    Level: 2    Role: D/V
 Verify that CI/CD pipelines use OIDC tokens from HashiCorp Vault, AWS IAM Roles, or Azure Managed Identity, ensuring their lifetimes do not exceed organizational security policy limits.
...
 Ensure that build environments operate in ephemeral containers or VMs with no persistent storage, and have network isolation from production VPCs.
...
 Verify that Kubernetes NetworkPolicies or any equivalent implement default-deny ingress and egress policies with explicit allow rules for required ports (443, 8080, etc.).
...
 Verify that AI infrastructure operates within dedicated VPCs/VNets without direct internet access and communicates exclusively through NAT gateways or bastion hosts.
...
 Ensure that container images are scanned using tools like GitLeaks, TruffleHog, or detect-secrets to block builds containing API keys, passwords, or certificates.
 #4.4.5    Level: 2    Role: D/V
 Verify that production secret access requires MFA using hardware tokens (YubiKey, FIDO2) and is logged in immutable audit records with user identities and timestamps.
 #4.4.6    Level: 2    Role: D/V
 Verify that secrets are injected through Kubernetes secrets, mounted volumes, or init containers, and ensure that secrets are never embedded in environment variables or container images.
...
 Verify that sandbox environments have no network connectivity (--network=none) or allow only localhost access, with all external requests blocked by iptables rules.
 #4.5.3    Level: 2    Role: D/V
 Ensure that AI model validation incorporates automated red-team testing with organizationally defined test coverage and behavioral analysis for detecting backdoors.
...
 Ensure that container images are scanned according to organizational schedules, with CRITICAL vulnerabilities blocking deployment based on organizational risk thresholds.
...
 Ensure that HIGH severity vulnerabilities are patched in accordance with organizational risk management timelines, including emergency procedures for actively exploited CVEs.
...
 Verify that GPU/TPU utilization is monitored, with alerts triggered at thresholds defined by the organization, and that automatic scaling or load balancing is activated according to capacity management policies.
...
 Ensure that capacity planning utilizes historical data with forecasting periods defined by the organization and automated resource provisioning based on demand patterns.
...
 Ensure that the dev, test, and prod environments operate within separate VPCs/VNets, without sharing IAM roles, security groups, or network connectivity.
...
 Verify that infrastructure-as-code changes undergo peer review, automated testing, and security scanning before merging into the main branch.
...
 Verify that promotion gates include automated security tests (SAST, DAST, container scanning) and require zero CRITICAL findings for approval.
...
 Verify that infrastructure configurations are backed up in accordance with organizational backup schedules, using the 3-2-1 backup strategy to store copies in geographically separate regions.
...
 Ensure that disaster recovery plans include AI-specific runbooks covering model weight restoration, GPU cluster rebuilding, and service dependency mapping.
...
Secure AI-specific hardware components such as GPUs, TPUs, and specialized AI accelerators.

 #4.11.1    Level: 2    Role: D/V
 Verify that AI accelerator firmware (GPU BIOS, TPU firmware) is validated using cryptographic signatures and updated according to the organization's patch management schedule.
 #4.11.2    Level: 2    Role: D/V
 Confirm that before workload execution, the AI accelerator's integrity is validated through hardware attestation using TPM 2.0, Intel TXT, or AMD SVM.
 #4.11.3    Level: 2    Role: D/V
 Verify that GPU memory is isolated between workloads using SR-IOV, MIG (Multi-Instance GPU), or equivalent hardware partitioning, ensuring memory sanitization between jobs.
 #4.11.4    Level: 3    Role: V
 Ensure the AI hardware supply chain includes provenance verification through manufacturer certificates and tamper-evident packaging validation.
...
 Verify that edge AI devices authenticate with the central infrastructure using mutual TLS, with device certificates rotated according to the organization's certificate management policy.
...
 Ensure that hybrid cloud AI workloads enforce consistent security policies across both on-premises and cloud environments, supported by unified monitoring and alerting.
...
 Ensure that GitOps repositories require signed commits using GPG keys and have branch protection rules that prevent direct pushes to main branches.
...
 Ensure that automated infrastructure provisioning includes security policy validation with deployment blocking for configurations that are non-compliant.
 #4.14.4    Level: 2    Role: D/V
 Ensure that infrastructure automation secrets are managed using external secret operators (such as External Secrets Operator, Bank-Vaults) with automatic rotation enabled.
 #4.14.5    Level: 3    Role: V
 Verify that the self-healing infrastructure includes security event correlation combined with automated incident response and stakeholder notification workflows.
...
 Verify that hybrid classical-quantum cryptographic systems provide defense-in-depth during the quantum transition period by implementing performance monitoring.
...
 Ensure that sensitive AI models run inside Intel SGX enclaves, AMD SEV-SNP, or ARM TrustZone environments with encrypted memory and attestation verification.
...
 Ensure that remote attestation confirms enclave integrity by providing cryptographic proof of the execution environment's authenticity before loading AI models.
 #4.16.4    Level: 3    Role: D/V
 Ensure that confidential AI inference services prevent model extraction by utilizing encrypted computation, sealed model weights, and secure execution.
...
 Ensure that private set intersection (PSI) protocols facilitate secure data matching in federated AI without revealing individual datasets.
 #4.17.4    Level: 3    Role: D/V
 Verify that zero-knowledge machine learning (ZKML) systems enable verifiable AI inferences supported by cryptographic proof of correct computation.
...
Protect AI infrastructure against timing, power, electromagnetic, and cache-based side-channel attacks that may leak sensitive information.
...
 Verify that neuromorphic chip security includes spike pattern encryption, synaptic weight protection, and hardware-based validation of learning rules.
...
 Ensure that privacy-preserving key management incorporates distributed key generation, threshold cryptography, and secure key rotation with hardware-backed protection.
...
 Verify that cloud storage integration employs end-to-end encryption with key management controlled by the agent.
 #4.15.2    Level: 2    Role: D/V
 Ensure that the security boundaries of hybrid deployments are clearly defined and that communication channels are encrypted.
...
## C5 Access Control and Identity Management for AI Components and Users
...
 Ensure that new principals complete identity proofing aligned with NIST 800-63-3 IAL-2 or equivalent standards before being granted access to the production system.
...
 Ensure that federated AI agents authenticate using signed JWT assertions with a maximum lifespan of 24 hours and include cryptographic proof of origin.
...
 Ensure that least-privilege principles are enforced by default for service accounts, beginning with read-only permissions, and require documented business justification for any write access.
...
 Ensure that data classification labels (PII, PHI, export-controlled, proprietary) automatically propagate to derived resources (embeddings, prompt caches, model outputs) with consistent policy enforcement.
...
 Verify that policy definitions are version-controlled, peer-reviewed, and validated through automated testing within CI/CD pipelines before deployment to production.
 #5.3.4    Level: 2    Role: V
 Ensure that policy evaluation results contain structured decision rationales and are transmitted to SIEM systems for correlation analysis and compliance reporting.
...
 Ensure that every API request includes an authenticated tenant identifier that is cryptographically verified against the session context and user entitlements.
...
 Ensure that high-risk capabilities (file system access, code execution, external API calls, financial transactions) are disabled by default and require explicit authorization for activation, accompanied by business justifications.
...
 Ensure that agent-initiated actions receive secondary authorization via the ABAC policy engine, including complete context evaluation and audit logging.
 #5.7.5    Level: 3    Role: V
 Ensure that agent error conditions and exception handling include capability scope information to support incident analysis and forensic investigation.
...
 Verify that static code analysis runs on forked or vendored machine learning libraries.
...
 Ensure that runtime sensors detect and alert on unexpected dynamic library loads that deviate from the signed SBOM.
...
 Verify that immutable digests are used rather than mutable tags in container references.
...
 Ensure that Sigstore/Cosign signatures verify the publisher’s identity before artifacts are cached locally.
...
 Verify that repository allow lists are reviewed quarterly, with documentation providing a business justification for each entry.
...
 Ensure that disallowed content (such as copyright material and PII) is removed through automated scrubbing before training.
...
 Verify that CI/CD audit logs are streamed to SIEM to detect anomalous package pulls or tampered build steps.
...
 Verify that ML-BOM generation and Cosign signing are automated in continuous integration (CI) and required for merging.
 #6.7.3    Level: 2    Role: D
 Ensure that ML-BOM completeness checks cause the build to fail if any component metadata (such as hash or license) is missing.
 #6.7.4    Level: 2    Role: V
 Confirm that downstream users can query ML-BOMs through the API to validate imported models at deployment time.
...
 Ensure that responses falling below a configurable confidence threshold activate fallback workflows (e.g., retrieval-augmented generation, secondary model, or human review).
 #7.2.3    Level: 2    Role: D/V
 Ensure that hallucination incidents are tagged with root-cause metadata and are fed into post-mortem and fine-tuning pipelines.
...
 Verify that pre-generation and post-generation classifiers block hate speech, harassment, self-harm, extremist content, and sexually explicit material in accordance with policy.
 #7.3.2    Level: 1    Role: D/V
 Verify that PII/PCI detection and automatic redaction are applied to every response; violations trigger a privacy incident.
 #7.3.3    Level: 2    Role: D
 Ensure that confidentiality tags (e.g., trade secrets) are consistently applied across all modalities to prevent leaks in text, images, or code.
...
 Verify that per-user and per-API-key quotas effectively limit requests, tokens, and costs, implementing exponential back-off on 429 errors.
 #7.4.2    Level: 1    Role: D/V
 Verify that privileged actions (file writes, code execution, network calls) require policy-based approval or human-in-the-loop review.
...
 Verify that the agent delegation depth, recursion limits, and allowed tool lists are explicitly configured.
 #7.4.5    Level: 3    Role: V
 Verify that violating limits generates structured security events for SIEM ingestion.
...
 Ensure that user-facing confidence scores or brief reasoning summaries are displayed when the risk assessment deems them appropriate.
 #7.5.2    Level: 2    Role: D/V
 Verify that the generated explanations do not reveal sensitive system prompts or proprietary data.
...
 Verify that the metrics (schema violations, hallucination rate, toxicity, PII leaks, latency, cost) are being streamed to a central monitoring platform.
...
 Verify that prompts are filtered to prevent attempts to generate impersonations, sexually explicit deepfakes, or media depicting real individuals without their consent.
...
 Verify that API keys or JWTs include scoped claims (e.g., collection IDs, action verbs) and are rotated at least quarterly.
 #8.1.3    Level: 2    Role: D/V
 Verify that privilege escalation attempts (e.g., cross-namespace similarity queries) are detected and logged in a SIEM within 5 minutes.
...
 Verify that PII and regulated data are detected through automated classifiers and are masked, tokenized, or dropped before embedding.
 #8.2.2    Level: 1    Role: D
 Ensure embedding pipelines reject or quarantine inputs containing executable code or non-UTF-8 artifacts that could compromise the index.
...
 Verify that logical deletions are followed by cryptographic shredding of storage blocks if supported by the hardware, or by destruction of the key-vault key.
...
 Ensure that application-layer encryption or searchable encryption protects vectors from being directly accessed by infrastructure administrators or cloud personnel.
 #8.4.3    Level: 3    Role: V
 Verify that the defense parameters (ε for DP, noise σ, projection rank k) achieve a balance of privacy with at least 99% token protection and utility with no more than 3% accuracy loss.
...
 Ensure that every retrieval query is post-filtered by tenant/user ID before being passed to the LLM prompt.
 #8.5.2    Level: 1    Role: D
 Verify that collection names or namespaced IDs are salted per user or tenant to prevent vector collisions across scopes.
...
 Verify that memory forgetting mechanisms securely delete sensitive information with cryptographic erasure guarantees by using key deletion, multi-pass overwriting, or hardware-based secure deletion accompanied by verification certificates.
...
 Ensure that the maximum recursion depth, breadth, wall-clock time, tokens, and monetary cost per agent execution are configured centrally and under version control.
...
 Verify that real-time resource monitors activate the circuit breaker interruption when any budget threshold is exceeded, stopping further task expansion.
 #9.1.4    Level: 2    Role: D/V
 Ensure that circuit-breaker events are logged with the agent ID, triggering condition, and captured plan state for forensic review.
...
Isolate tool interactions to prevent unauthorized access to the system or the execution of code.

 #9.2.1    Level: 1    Role: D/V
 Ensure that every tool or plugin runs within an OS, container, or WASM-level sandbox with least-privilege file system, network, and system call policies.
...
 Ensure that formal analysis or model checking verifies the absence of unbounded recursion in agent protocols.
 #9.3.5    Level: 3    Role: D
 Verify that loop-abort events generate alerts and provide continuous-improvement metrics.

---

### 9.4 Protection Against Misuse at the Protocol Level
...
 Ensure that all agent-to-tool and agent-to-agent messages are authenticated (e.g., mutual TLS or JWT) and end-to-end encrypted.
 #9.4.2    Level: 1    Role: D
 Ensure that schemas are strictly validated; unknown fields or malformed messages are rejected.
...
 Ensure that tamper-evident logs are stored in an append-only or write-once storage medium.
...
 Ensure that agents authenticate as first-class principals with downstream systems, never reusing end-user credentials.
...
 Verify that message integrity and origin are validated before processing; any failures trigger alerts and cause the message to be dropped.
...
 Verify that transitions in reasoning strategies are logged with complete context, including input characteristics, selection criteria values, and execution metadata, to enable audit trail reconstruction.
 #9.10.4    Level: 2    Role: D/V
 Verify that Tree-of-Thoughts reasoning incorporates branch pruning mechanisms to terminate exploration when policy violations, resource limits, or safety boundaries are detected.
 #9.10.5    Level: 2    Role: D/V
 Verify that ReAct (Reason-Act-Observe) cycles include validation checkpoints at each phase: reasoning step verification, action authorization, and observation sanitization before moving forward.
 #9.10.6    Level: 3    Role: D/V
 Ensure that performance metrics for reasoning strategies (execution time, resource usage, output quality) are monitored with automated alerts triggered when metrics exceed configured thresholds.
 #9.10.7    Level: 3    Role: D/V
 Ensure that hybrid reasoning approaches, which combine multiple strategies, uphold input validation and output constraints for all constituent strategies without circumventing any security controls.
...
 Verify that agent initialization includes establishing a cryptographic identity using hardware-backed credentials and generating immutable startup audit logs containing the agent ID, timestamp, configuration hash, and initialization parameters.
...
 Verify that agent recovery mechanisms validate state integrity using cryptographic checksums (at least SHA-256) and roll back to known-good states when corruption is detected, with automated alerts and requirements for manual approval.
...
 Verify that tool execution results are validated against the expected schemas (JSON Schema, XML Schema) and security policies (output sanitization, data classification) before integration, including timeout limits and error-handling procedures.
...
 Verify that tool security assessments are automatically triggered for new versions, with mandatory approval gates that include static analysis, dynamic testing, and a security team review, all supported by documented approval criteria and SLA requirements.
...
 Verify that an alignment test suite (red team prompts, jailbreak probes, disallowed content) is version-controlled and executed for every model release.
...
### 10.2 Hardening Against Adversarial Examples

Increase resilience to manipulated inputs. Robust adversarial training and benchmark scoring are currently considered best practices.
...
### 10.3 Mitigation of Membership Inference
...
 Verify that query rate limits restrict repeated adaptive queries from the same principal.
...
 Verify that query entropy and input plurality statistics support an automated extraction detector.
...
 Verify that inputs pass through an anomaly detector (e.g., STRIP, consistency scoring) before performing model inference.
...
 Ensure that security policies can be updated dynamically without restarting the agent, while preserving the integrity of the policy versions.
...
 Confirm that continuous learning from security reflections enhances threat detection without compromising legitimate functionality.
...
 Verify that recursive self-improvement is limited by formal safety constraints with mathematical proofs of convergence.
...
 Ensure that automated audits measure k-anonymity and l-diversity and trigger alerts when thresholds fall below policy limits.
...
 Verify that formal proofs or synthetic data certification demonstrate a re-identification risk of ≤ 0.05, even in the presence of linkage attacks.
...
 Verify that privacy-loss accounting dashboards issue alerts when cumulative ε exceeds policy thresholds.
 #11.3.2    Level: 2    Role: V
 Verify that black-box privacy audits estimate ε̂ to within 10% of the declared value.
...
 Verify that every dataset and model checkpoint includes a machine-readable purpose tag aligned with the original consent.
...
 Verify that APIs expose consent tokens; models must validate the token scope before performing inference.
...
 Ensure that training metrics maintain differential privacy and never disclose individual client loss.
...
This section outlines the requirements for providing real-time and forensic visibility into what the model and other AI components observe, perform, and return, enabling the detection, triage, and learning from threats.

### C12.1 Request and Response Logging

 #12.1.1    Level: 1    Role: D/V
 Ensure that all user prompts and model responses are recorded with suitable metadata, such as timestamp, user ID, session ID, and model version.
...
 Verify that policy decisions and safety filtering actions are logged with sufficient detail to allow auditing and debugging of content moderation systems.
...
 Verify that the system identifies and alerts on known jailbreak patterns, prompt injection attempts, and adversarial inputs using signature-based detection.
...
 Ensure the system tracks essential performance metrics such as accuracy, confidence scores, latency, and error rates across different model versions and time periods.
...
 Verify that hallucination detection monitors recognize and flag instances when model outputs include factually incorrect, inconsistent, or fabricated information.
...
 Verify that resource utilization monitoring includes GPU and CPU usage, memory consumption, and storage requirements, with alerts triggered when thresholds are exceeded.
...
 Ensure that incident response plans specifically address AI-related security events, including model compromise, data poisoning, and adversarial attacks.
...
### C12.5 Detection of AI Performance Degradation
...
 Verify that DAG visualization data is sanitized to remove any sensitive information before storage or transmission.
...
 Ensure that DAG data integrity is safeguarded through cryptographic signatures and tamper-evident storage mechanisms.
...
 Ensure that proactive agent behaviors are security-validated before execution by integrating risk assessment.
...
 Ensure that security-critical proactive actions require explicit approval chains complete with audit trails.
...
 Ensure that audit logs cannot be tampered with and include mechanisms for verifying their integrity.
...
 Verify that performance metrics for different applicable use cases are disclosed.
...
 Ensure that AI systems provide confidence scores or uncertainty measurements with their outputs.
...
 Verify that transparency reports published regularly disclose AI incidents and operational metrics with reasonable detail.
...
 Confirm that an AI-specific risk assessment methodology is documented and implemented.
 #AC.1.2    Level: 2    Role: D
 Ensure that risk assessments are conducted at critical points in the AI lifecycle and before any significant changes.
...
 Verify the existence of documented AI security policies.
 #AC.2.2    Level: 2    Role: D
 Ensure that policies are reviewed and updated at least annually and following any significant changes in the threat landscape.
...
 Ensure that AI security roles and responsibilities are documented.
...
 Confirm that ethical guidelines for AI development and deployment are established.
...
 Verify that regulatory changes prompt timely reviews and updates of AI systems.

### AC.6 Training Data Governance, Documentation, and Process

 #1.1.2    Level: 1    Role: D/V
 Ensure that only datasets reviewed for quality, representativeness, ethical sourcing, and license compliance are used, minimizing the risks of poisoning, embedded bias, and intellectual property infringement.
...
 Ensure that "data cards" or "datasheets for datasets" are maintained for significant training datasets, documenting their characteristics, motivations, composition, collection processes, preprocessing, and recommended or discouraged uses.
 #1.3.2    Level: 2    Role: D/V
 Verify that identified biases are mitigated through documented strategies such as re-balancing, targeted data augmentation, algorithmic adjustments (e.g., pre-processing, in-processing, post-processing techniques), or re-weighting, and ensure the impact of these mitigations on both fairness and overall model performance is assessed.
...
 Ensure labeling and annotation quality by implementing clear guidelines, conducting reviewer cross-checks, utilizing consensus mechanisms (such as monitoring inter-annotator agreement), and establishing defined processes for resolving discrepancies.
...
 Ensure that manual spot-checks conducted by domain experts cover a statistically significant sample size (e.g., at least 1% or 1,000 samples, whichever is larger, or as determined by a risk assessment) to detect subtle quality issues that automation may miss.
 #1.8.4    Level: 2    Role: D/V
 Ensure that outsourced or crowdsourced labeling workflows incorporate technical and procedural safeguards to maintain data confidentiality, integrity, label quality, and to prevent data leakage.
...
 Verify that the results contribute to the model’s security dossier and support ongoing threat intelligence.
...
 Verify that training data deletion workflows remove both primary and derived data, assess the impact on models, and ensure that any effects on affected models are evaluated and, if necessary, addressed (e.g., through retraining or recalibration).
...
 Ensure that high-risk data sources (e.g., open-source datasets with unknown provenance or unvetted suppliers) undergo enhanced scrutiny, such as sandboxed analysis, thorough quality and bias checks, and targeted poisoning detection, before being used in sensitive applications.
...
 Ensure that the effects of adversarial robustness training on model performance—against both clean and adversarial inputs—as well as fairness metrics, are evaluated, documented, and continuously monitored.
...
 #1.4.2    Level: 2    Role: D/V
 Ensure that failed datasets are quarantined with audit trails.
...
 Verify that synthetic data has been risk-assessed for bias, privacy leakage, and representational issues before being used in training.
...
 Verify that retention and deletion actions are properly logged and auditable.
...
 Verify that sector-specific regulations (e.g., healthcare, finance) are properly identified and addressed in data handling.
...
 Ensure that the results of the impact analysis are documented and reviewed by the relevant stakeholders.
...
 Ensure that the documented workflow specifies when and how AI tools are used to generate, refactor, or review code.
 #AD.1.2    Level: 2    Role: D
 Verify that the workflow corresponds to each SSDLC phase: design, implementation, code review, testing, and deployment.
...
 Ensure that evaluations adhere to a recognized framework and are re-conducted after major version changes.
...
 Ensure that AI-generated code is always reviewed by a human coder.
...
 Verify that explainability reports are stored alongside design artifacts and referenced in security reviews, ensuring compliance with ISO/IEC 42001 traceability principles.
...
 Ensure that aggregated feedback is used to guide periodic fine-tuning or retrieval-augmented generation with vetted secure-coding corpora (e.g., OWASP Cheat Sheets).
 #AD.6.3    Level: 3    Role: D/V
 Verify that a closed-loop evaluation system runs regression tests after every fine-tuning; security metrics must meet or exceed previous baselines before deployment.
...
 #AE.1.2    Section: 1.2
 Encryption In Transit Use TLS for HTTPS-based applications, using tools such as OpenSSL and Python's`ssl`library.
