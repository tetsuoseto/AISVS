...
| :----: | --------------------------------------------------------------------------------------------------------------------------------------------------------- | :---: | :--: |
| 10.1.1 | Verify that an alignment test suite (red-team prompts, jailbreak probes, disallowed content) is version-controlled and executed with every model release. |   1   | D/V  |
...
| 10.1.4 | Ensure that counter-jailbreak training is properly documented and can be consistently reproduced.                                                         |   2   |  D   |
| 10.1.5 | Ensure that formal proofs of policy compliance or certified monitoring address critical domains.                                                          |   3   |  V   |
...
| 10.2.5 | Verify that regression tests utilize adaptive attacks to ensure there is no measurable loss of robustness.       |   3   |  V   |
...
|   #    | Description                                                                                                            | Level | Role |
| :----: | ---------------------------------------------------------------------------------------------------------------------- | :---: | :--: |
| 10.4.1 | Verify that sensitive attributes are never directly outputted; when necessary, use buckets or one-way transformations. |   1   |  D   |
| 10.4.2 | Confirm that query rate limits effectively throttle repeated adaptive queries from the same principal.                 |   1   | D/V  |
...
| 10.5.3 | Verify that fragile or probabilistic watermarks can be proven with p < 0.01 in 1,000 or fewer queries against a suspected clone. |   2   |  V   |
...
| 10.6.2 | Verify that the detector thresholds are adjusted on clean and poisoned validation sets to achieve less than 5% false positives. |   1   |  V   |
| 10.6.3 | Verify that inputs identified as poisoned trigger soft-blocking and human review workflows.                                     |   2   |  D   |
| 10.6.4 | Verify that detectors are stress-tested using adaptive, triggerless backdoor attacks.                                           |   2   |  V   |
| 10.6.5 | Ensure that detection efficacy metrics are logged and periodically re-evaluated using updated threat intelligence.              |   3   |  D   |
...
| 10.7.2 | Verify that policy updates are cryptographically signed by authorized security personnel and validated before being applied.                 |   2   | D/V  |
...
| 10.8.3 | Verify that meta-cognitive security analysis identifies potential biases, manipulation, or compromises in agent reasoning processes. |   2   | D/V  |
...
|   #    | Description                                                                                                                      | Level | Role |
| :----: | -------------------------------------------------------------------------------------------------------------------------------- | :---: | :--: |
| 10.9.1 | Verify that self-modification capabilities are limited to designated safe areas with formal verification boundaries.             |   1   | D/V  |
| 10.9.2 | Ensure that evolution proposals undergo a security impact assessment before implementation.                                      |   2   | D/V  |
| 10.9.3 | Ensure that self-improvement mechanisms incorporate rollback features with integrity verification.                               |   2   | D/V  |
