...
| :---: | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :---: | :--: |
| 9.1.1 | Ensure that the maximum recursion depth, breadth, wall-clock time, tokens, and monetary cost per agent execution are configured centrally and under version control.    |   1   | D/V  |
...
| 9.1.4 | Ensure that circuit-breaker events are logged with the agent ID, triggering condition, and captured plan state for forensic review.                                     |   2   | D/V  |
...
|   #   | Description                                                                                                                                               | Level | Role |
| :---: | --------------------------------------------------------------------------------------------------------------------------------------------------------- | :---: | :--: |
| 9.2.1 | Ensure that every tool or plugin runs within an OS, container, or WASM-level sandbox with least-privilege file system, network, and system call policies. |   1   | D/V  |
...
| 9.3.4 | Ensure that formal analysis or model checking verifies the absence of unbounded recursion in agent protocols.               |   3   |  V   |
...
|   #   | Description                                                                                                                           | Level | Role |
| :---: | ------------------------------------------------------------------------------------------------------------------------------------- | :---: | :--: |
| 9.4.1 | Ensure that all agent-to-tool and agent-to-agent messages are authenticated (e.g., mutual TLS or JWT) and end-to-end encrypted.       |   1   | D/V  |
| 9.4.2 | Ensure that schemas are strictly validated; unknown fields or malformed messages are rejected.                                        |   1   |  D   |
...
| 9.5.3 | Ensure that tamper-evident logs are stored in an append-only or write-once storage medium.                        |   2   |  V   |
...
|   #   | Description                                                                                                                   | Level | Role |
| :---: | ----------------------------------------------------------------------------------------------------------------------------- | :---: | :--: |
| 9.7.1 | Ensure that agents authenticate as first-class principals with downstream systems, never reusing end-user credentials.        |   1   | D/V  |
...
| 9.8.2 | Verify that message integrity and origin are validated before processing; any failures trigger alerts and cause the message to be dropped. |   1   |  D   |
...
| 9.10.3 | Verify that transitions in reasoning strategies are logged with complete context, including input characteristics, selection criteria values, and execution metadata, to enable audit trail reconstruction.                     |   2   | D/V  |
| 9.10.4 | Verify that Tree-of-Thoughts reasoning incorporates branch pruning mechanisms to terminate exploration when policy violations, resource limits, or safety boundaries are detected.                                              |   2   | D/V  |
...
| 9.10.6 | Ensure that performance metrics for reasoning strategies (execution time, resource usage, output quality) are monitored with automated alerts triggered when metrics exceed configured thresholds.                              |   3   | D/V  |
| 9.10.7 | Ensure that hybrid reasoning approaches, which combine multiple strategies, uphold input validation and output constraints for all constituent strategies without circumventing any security controls.                          |   3   | D/V  |
...
|   #    | Description                                                                                                                                                                                                                                                      | Level | Role |
| :----: | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :---: | :--: |
| 9.11.1 | Verify that agent initialization includes establishing a cryptographic identity using hardware-backed credentials and generating immutable startup audit logs containing the agent ID, timestamp, configuration hash, and initialization parameters.             |   1   | D/V  |
...
| 9.11.4 | Verify that agent recovery mechanisms validate state integrity using cryptographic checksums (at least SHA-256) and roll back to known-good states when corruption is detected, with automated alerts and requirements for manual approval.                      |   3   | D/V  |
...
| 9.12.2 | Verify that tool execution results are validated against the expected schemas (JSON Schema, XML Schema) and security policies (output sanitization, data classification) before integration, including timeout limits and error-handling procedures.             |   1   | D/V  |
...
| 9.12.5 | Verify that tool security assessments are automatically triggered for new versions, with mandatory approval gates that include static analysis, dynamic testing, and a security team review, all supported by documented approval criteria and SLA requirements. |   3   | D/V  |
