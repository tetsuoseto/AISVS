...
| 8.1.2 | Ensure that API keys or JWTs contain scoped claims (e.g., collection IDs, action verbs) and are rotated at least quarterly.                           |   1   | D/V  |
...
| 8.1.4 | Verify that the vector database audit logs include subject identifier, operation, vector ID/namespace, similarity threshold, and result count.        |   2   | D/V  |
...
|   #   | Description                                                                                                                                                                      | Level | Role |
| :---: | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :---: | :--: |
| 8.2.1 | Verify that Personally Identifiable Information (PII) and regulated data are detected using automated classifiers and are masked, tokenized, or removed before embedding.        |   1   | D/V  |
| 8.2.2 | Ensure that embedding pipelines reject or quarantine inputs containing executable code or non-UTF-8 artifacts that could corrupt the index.                                      |   1   |  D   |
...
|   #   | Description                                                                                                                                            | Level | Role |
| :---: | ------------------------------------------------------------------------------------------------------------------------------------------------------ | :---: | :--: |
| 8.3.1 | Verify that every vector and metadata record includes a TTL or an explicit retention label that is respected by automated cleanup jobs.                |   1   | D/V  |
...
| 8.3.3 | Verify that logical deletions are followed by cryptographic shredding of storage blocks if supported by hardware, or by destruction of key-vault keys. |   2   |  D   |
...
|   #   | Description                                                                                                                                                              | Level | Role |
| :---: | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | :---: | :--: |
| 8.4.1 | Verify that a formal threat model addressing inversion, membership, and attribute-inference attacks exists and is reviewed annually.                                     |   1   |  V   |
...
| 8.5.2 | Verify that collection names or namespaced IDs are uniquely salted for each user or tenant to prevent vectors from colliding across different scopes. |   1   |  D   |
...
|   #   | Description                                                                                                                                                                                                                                            | Level | Role |
| :---: | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | :---: | :--: |
| 8.6.1 | Verify that different memory types (episodic, semantic, working) have isolated security contexts, role-based access controls, separate encryption keys, and documented access patterns for each memory type.                                           |   1   | D/V  |
...
| 8.6.5 | Ensure that memory system integrity is continuously monitored for unauthorized modifications or corruption by using checksums, audit logs, and automated alerts when memory content changes occur outside of normal operations.                        |   3   | D/V  |
