...
| :---: | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :---: | :--: |
| 4.1.1 | Ensure that all AI containers drop ALL Linux capabilities except for CAP_SETUID, CAP_SETGID, and any explicitly required capabilities documented in the security baselines.                                                             |   1   | D/V  |
...
| 4.1.3 | Ensure that AI workloads operate with read-only root filesystems, use tmpfs for temporary data, and utilize named volumes for persistent data with noexec mount options enforced.                                                       |   2   | D/V  |
...
|   #   | Description                                                                                                                                                                                 | Level | Role |
| :---: | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :---: | :--: |
| 4.2.1 | Ensure that infrastructure-as-code is scanned with tools (tfsec, Checkov, or Terrascan) on every commit, blocking merges if CRITICAL or HIGH severity findings are detected.                |   1   | D/V  |
| 4.2.2 | Ensure that container builds are reproducible with identical SHA256 hashes across builds and generate SLSA Level 3 provenance attestations signed using Sigstore.                           |   1   | D/V  |
...
|   #   | Description                                                                                                                                                                                  | Level | Role |
| :---: | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :---: | :--: |
| 4.3.1 | Verify that Kubernetes NetworkPolicies or any equivalent enforce a default-deny for ingress and egress, with explicit allow rules for required ports (443, 8080, etc.).                      |   1   | D/V  |
...
| 4.3.4 | Verify that inter-service communication uses mutual TLS with certificates rotated according to the organizational policy and that certificate validation is enforced (no skip-verify flags). |   2   | D/V  |
...
| 4.4.4 | Ensure that container images are scanned using tools such as GitLeaks, TruffleHog, or detect-secrets to block builds that contain API keys, passwords, or certificates.                                                   |   2   | D/V  |
| 4.4.5 | Verify that production secret access requires multi-factor authentication (MFA) using hardware tokens (YubiKey, FIDO2) and that access is recorded in immutable audit logs with user identities and timestamps.           |   2   | D/V  |
...
|   #   | Description                                                                                                                                                                          | Level | Role |
| :---: | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | :---: | :--: |
| 4.5.1 | Verify that external AI models run in gVisor, microVMs (such as Firecracker, CrossVM), or Docker containers with the --security-opt=no-new-privileges and --read-only flags enabled. |   1   | D/V  |
...
| 4.5.3 | Ensure that AI model validation includes automated red-team testing with organization-defined test coverage and behavioral analysis for detecting backdoors.                         |   2   | D/V  |
| 4.5.4 | Verify that before an AI model is deployed to production, its sandbox results are cryptographically signed by authorized security personnel and stored in immutable audit logs.      |   2   | D/V  |
| 4.5.5 | Ensure that sandbox environments are destroyed and recreated from golden images between evaluations, including complete filesystem and memory cleanup.                               |   2   | D/V  |
...
| 4.6.2 | Verify that the infrastructure meets CIS Benchmarks or NIST 800-53 controls with organization-defined compliance thresholds and automated remediation for failed checks.                                |   1   | D/V  |
...
| 4.6.6 | Verify that configuration drift is detected using tools such as Chef InSpec and AWS Config in accordance with organizational monitoring requirements, with automatic rollback for unauthorized changes. |   2   | D/V  |
...
| 4.7.3 | Verify that Kubernetes ResourceQuotas or their equivalents limit individual workloads according to organizational resource allocation policies, with hard limits enforced.                               |   2   | D/V  |
| 4.7.4 | Verify that cost monitoring tracks spending for each workload/tenant, includes alerts based on organizational budget thresholds, and implements automated controls to prevent budget overruns.           |   2   |  V   |
| 4.7.5 | Confirm that capacity planning utilizes historical data with forecasting periods defined by the organization and automated resource provisioning based on demand patterns.                               |   3   |  V   |
...
|   #   | Description                                                                                                                                                                               | Level | Role |
| :---: | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :---: | :--: |
| 4.8.1 | Verify that development, test, and production environments operate in separate VPCs/VNets with no shared IAM roles, security groups, or network connectivity.                             |   1   | D/V  |
| 4.8.2 | Verify that environment promotion requires approval from organizationally authorized personnel using cryptographic signatures and maintains immutable audit trails.                       |   1   | D/V  |
...
| 4.8.5 | Verify that non-production data is anonymized in accordance with organizational privacy requirements, using synthetic data generation or complete data masking with PII removal verified. |   2   | D/V  |
...
|   #   | Description                                                                                                                                                                                      | Level | Role |
| :---: | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | :---: | :--: |
| 4.9.1 | Verify that infrastructure configurations are backed up according to organizational backup schedules to geographically separate regions, implementing the 3-2-1 backup strategy.                 |   1   | D/V  |
| 4.9.2 | Verify that backup systems operate on isolated networks with separate credentials and use air-gapped storage for ransomware protection.                                                          |   2   | D/V  |
| 4.9.3 | Verify that recovery procedures are tested and validated through automated testing according to the organizational schedule, ensuring that RTO and RPO targets meet organizational requirements. |   2   |  V   |
| 4.9.4 | Verify that disaster recovery includes AI-specific runbooks covering model weight restoration, GPU cluster rebuilding, and service dependency mapping.                                           |   3   |  V   |
...
|   #    | Description                                                                                                                                                                          | Level | Role |
| :----: | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | :---: | :--: |
| 4.10.1 | Verify that infrastructure compliance is evaluated according to organizational schedules against SOC 2, ISO 27001, or FedRAMP controls, using automated evidence collection.         |   2   | D/V  |
| 4.10.2 | Ensure that infrastructure documentation includes network diagrams, data flow maps, and threat models, all updated in accordance with organizational change management requirements. |   2   |  V   |
...
|   #    | Description                                                                                                                                                                 | Level | Role |
| :----: | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :---: | :--: |
| 4.11.1 | Ensure that AI accelerator firmware (GPU BIOS, TPU firmware) is verified using cryptographic signatures and updated according to organizational patch management schedules. |   2   | D/V  |
...
|   #    | Description                                                                                                                                                                                       | Level | Role |
| :----: | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :---: | :--: |
| 4.12.1 | Verify that edge AI devices authenticate with the central infrastructure using mutual TLS and that device certificates are rotated according to the organizational certificate management policy. |   2   | D/V  |
...
| 4.12.3 | Verify that distributed AI coordination employs Byzantine fault-tolerant consensus algorithms with participant validation and malicious node detection.                                           |   3   | D/V  |
...
| 4.13.2 | Verify that cross-cloud data transfer employs end-to-end encryption using customer-managed keys and enforces data residency controls according to jurisdiction.     |   2   | D/V  |
| 4.13.3 | Ensure that hybrid cloud AI workloads apply consistent security policies across both on-premise and cloud environments through unified monitoring and alerting.     |   2   | D/V  |
...
| 4.13.5 | Verify that multi-cloud cost optimization includes security controls to prevent resource sprawl as well as unauthorized cross-cloud data transfer charges.          |   3   |  V   |
...
|   #    | Description                                                                                                                                                                                                   | Level | Role |
| :----: | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :---: | :--: |
| 4.14.1 | Ensure that GitOps repositories require commits to be signed with GPG keys and implement branch protection rules that prevent direct pushes to main branches.                                                 |   2   | D/V  |
| 4.14.2 | Ensure that infrastructure automation includes drift detection with automatic remediation and rollback capabilities triggered according to the organization's response requirements for unauthorized changes. |   2   | D/V  |
| 4.14.3 | Ensure that automated infrastructure provisioning includes validation of security policies, with deployment blocked for configurations that do not comply.                                                    |   2   | D/V  |
| 4.14.4 | Ensure that infrastructure automation secrets are managed through external secret operators (such as External Secrets Operator, Bank-Vaults) with automatic rotation.                                         |   2   | D/V  |
...
| 4.15.4 | Verify that quantum threat modeling evaluates AI infrastructure vulnerability to quantum attacks and includes documented migration timelines and risk assessments.                   |   3   |  V   |
...
| 4.16.2 | Ensure that confidential containers (such as Kata Containers and gVisor with confidential computing) isolate AI workloads using hardware-enforced memory encryption. |   3   | D/V  |
| 4.16.3 | Verify that remote attestation confirms enclave integrity before loading AI models by providing cryptographic proof of the execution environment's authenticity.     |   3   | D/V  |
| 4.16.4 | Ensure that confidential AI inference services prevent model extraction by using encrypted computation with sealed model weights and secure execution.               |   3   | D/V  |
| 4.16.5 | Verify that the trusted execution environment orchestration manages the secure enclave lifecycle through remote attestation and encrypted communication channels.    |   3   | D/V  |
...
| 4.17.3 | Ensure that private set intersection (PSI) protocols enable secure data matching in federated AI without revealing individual datasets.                                   |   3   | D/V  |
...
| 4.19.2 | Ensure that FPGA-based AI accelerators implement bitstream encryption, anti-tamper mechanisms, and secure configuration loading with authenticated updates. |   3   | D/V  |
...
| 4.20.5 | Ensure that privacy-preserving computation performance is optimized through batching, caching, and hardware acceleration while maintaining cryptographic security guarantees.             |   3   | D/V  |
...
| 4.15.2 | Ensure that the security boundaries in a hybrid deployment are clearly defined and that communication channels are encrypted. |   2   | D/V  |
...
| 4.15.5 | Verify that cloud provider security assessments include threat modeling and risk evaluation specific to each agent.           |   3   | D/V  |
