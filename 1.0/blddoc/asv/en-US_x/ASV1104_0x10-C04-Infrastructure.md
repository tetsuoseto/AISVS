...
| 4.1.2 | Verify that seccomp profiles block all syscalls except those on pre-approved allowlists, with any violations terminating the container and triggering security alerts.                                                                      |   1   | D/V  |
...
| 4.1.4 | Verify that eBPF-based runtime monitoring (such as Falco, Tetragon, or an equivalent solution) detects privilege escalation attempts and automatically terminates offending processes within the organization's response time requirements. |   2   | D/V  |
...
|   #   | Description                                                                                                                                                                               | Level | Role |
| :---: | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :---: | :--: |
| 4.2.1 | Ensure that infrastructure-as-code is scanned with tools (tfsec, Checkov, or Terrascan) on every commit, blocking merges that contain CRITICAL or HIGH severity findings.                 |   1   | D/V  |
| 4.2.2 | Verify that container builds are reproducible with identical SHA256 hashes across multiple builds and generate SLSA Level 3 provenance attestations signed using Sigstore.                |   1   | D/V  |
| 4.2.3 | Ensure that container images include embedded CycloneDX or SPDX SBOMs and are signed with Cosign before pushing to the registry, with unsigned images being rejected during deployment.   |   2   | D/V  |
| 4.2.4 | Verify that CI/CD pipelines use OIDC tokens from HashiCorp Vault, AWS IAM Roles, or Azure Managed Identity, ensuring their lifetimes do not exceed organizational security policy limits. |   2   | D/V  |
| 4.2.5 | Ensure that Cosign signatures and SLSA provenance are validated during the deployment process before container execution, and that any verification errors cause the deployment to fail.  |   2   | D/V  |
| 4.2.6 | Ensure that build environments operate in ephemeral containers or VMs with no persistent storage, and have network isolation from production VPCs.                                        |   2   | D/V  |
...
|   #   | Description                                                                                                                                                                              | Level | Role |
| :---: | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :---: | :--: |
| 4.3.1 | Verify that Kubernetes NetworkPolicies or any equivalent implement default-deny ingress and egress policies with explicit allow rules for required ports (443, 8080, etc.).              |   1   | D/V  |
...
| 4.3.3 | Verify that outbound traffic is filtered through HTTP/HTTPS proxies (such as Squid, Istio, or cloud NAT gateways), using domain allowlists, and that blocked requests are logged.        |   2   | D/V  |
...
| 4.3.5 | Verify that AI infrastructure operates within dedicated VPCs/VNets without direct internet access and communicates exclusively through NAT gateways or bastion hosts.                    |   2   | D/V  |
...
| 4.4.2 | Verify that cryptographic keys are generated in FIPS 140-2 Level 2 HSMs (AWS CloudHSM, Azure Dedicated HSM) and that key rotation is performed according to the organizational cryptographic policy. |   1   | D/V  |
| 4.4.3 | Verify that secret rotation is automated with zero-downtime deployment and immediate rotation triggered by personnel changes or security incidents.                                                  |   2   | D/V  |
| 4.4.4 | Ensure that container images are scanned using tools like GitLeaks, TruffleHog, or detect-secrets to block builds containing API keys, passwords, or certificates.                                   |   2   | D/V  |
| 4.4.5 | Verify that production secret access requires MFA using hardware tokens (YubiKey, FIDO2) and is logged in immutable audit records with user identities and timestamps.                               |   2   | D/V  |
| 4.4.6 | Verify that secrets are injected through Kubernetes secrets, mounted volumes, or init containers, and ensure that secrets are never embedded in environment variables or container images.           |   2   | D/V  |
...
| 4.5.2 | Verify that sandbox environments have no network connectivity (--network=none) or allow only localhost access, with all external requests blocked by iptables rules.            |   1   | D/V  |
| 4.5.3 | Ensure that AI model validation incorporates automated red-team testing with organizationally defined test coverage and behavioral analysis for detecting backdoors.            |   2   | D/V  |
...
|   #   | Description                                                                                                                                                                                         | Level | Role |
| :---: | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :---: | :--: |
| 4.6.1 | Ensure that container images are scanned according to organizational schedules, with CRITICAL vulnerabilities blocking deployment based on organizational risk thresholds.                          |   1   | D/V  |
...
| 4.6.3 | Ensure that HIGH severity vulnerabilities are patched in accordance with organizational risk management timelines, including emergency procedures for actively exploited CVEs.                      |   2   | D/V  |
...
|   #   | Description                                                                                                                                                                                                         | Level | Role |
| :---: | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :---: | :--: |
| 4.7.1 | Verify that GPU/TPU utilization is monitored, with alerts triggered at thresholds defined by the organization, and that automatic scaling or load balancing is activated according to capacity management policies. |   1   | D/V  |
...
| 4.7.5 | Ensure that capacity planning utilizes historical data with forecasting periods defined by the organization and automated resource provisioning based on demand patterns.                                           |   3   |  V   |
...
|   #   | Description                                                                                                                                                                                  | Level | Role |
| :---: | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :---: | :--: |
| 4.8.1 | Ensure that the dev, test, and prod environments operate within separate VPCs/VNets, without sharing IAM roles, security groups, or network connectivity.                                    |   1   | D/V  |
| 4.8.2 | Verify that environment promotion requires approval from organizationally defined authorized personnel using cryptographic signatures and immutable audit trails.                            |   1   | D/V  |
...
| 4.8.4 | Verify that infrastructure-as-code changes undergo peer review, automated testing, and security scanning before merging into the main branch.                                                |   2   | D/V  |
...
| 4.8.6 | Verify that promotion gates include automated security tests (SAST, DAST, container scanning) and require zero CRITICAL findings for approval.                                               |   2   | D/V  |
...
|   #   | Description                                                                                                                                                                                     | Level | Role |
| :---: | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :---: | :--: |
| 4.9.1 | Verify that infrastructure configurations are backed up in accordance with organizational backup schedules, using the 3-2-1 backup strategy to store copies in geographically separate regions. |   1   | D/V  |
| 4.9.2 | Ensure that backup systems operate within isolated networks using separate credentials and air-gapped storage to protect against ransomware.                                                    |   2   | D/V  |
...
| 4.9.4 | Ensure that disaster recovery plans include AI-specific runbooks covering model weight restoration, GPU cluster rebuilding, and service dependency mapping.                                     |   3   |  V   |
...
|   #    | Description                                                                                                                                                                     | Level | Role |
| :----: | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :---: | :--: |
| 4.11.1 | Verify that AI accelerator firmware (GPU BIOS, TPU firmware) is validated using cryptographic signatures and updated according to the organization's patch management schedule. |   2   | D/V  |
| 4.11.2 | Confirm that before workload execution, the AI accelerator's integrity is validated through hardware attestation using TPM 2.0, Intel TXT, or AMD SVM.                          |   2   | D/V  |
| 4.11.3 | Verify that GPU memory is isolated between workloads using SR-IOV, MIG (Multi-Instance GPU), or equivalent hardware partitioning, ensuring memory sanitization between jobs.    |   2   | D/V  |
| 4.11.4 | Ensure the AI hardware supply chain includes provenance verification through manufacturer certificates and tamper-evident packaging validation.                                 |   3   |  V   |
...
|   #    | Description                                                                                                                                                                                | Level | Role |
| :----: | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | :---: | :--: |
| 4.12.1 | Verify that edge AI devices authenticate with the central infrastructure using mutual TLS, with device certificates rotated according to the organization's certificate management policy. |   2   | D/V  |
| 4.12.2 | Verify that edge devices implement secure boot with verified signatures and rollback protection to prevent firmware downgrade attacks.                                                     |   2   | D/V  |
...
| 4.13.2 | Verify that cross-cloud data transfers use end-to-end encryption with customer-managed keys and that data residency controls are enforced according to jurisdiction.     |   2   | D/V  |
| 4.13.3 | Ensure that hybrid cloud AI workloads enforce consistent security policies across both on-premises and cloud environments, supported by unified monitoring and alerting. |   2   | D/V  |
...
|   #    | Description                                                                                                                                                                                               | Level | Role |
| :----: | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :---: | :--: |
| 4.14.1 | Ensure that GitOps repositories require signed commits using GPG keys and have branch protection rules that prevent direct pushes to main branches.                                                       |   2   | D/V  |
...
| 4.14.3 | Ensure that automated infrastructure provisioning includes security policy validation with deployment blocking for configurations that are non-compliant.                                                 |   2   | D/V  |
| 4.14.4 | Ensure that infrastructure automation secrets are managed using external secret operators (such as External Secrets Operator, Bank-Vaults) with automatic rotation enabled.                               |   2   | D/V  |
| 4.14.5 | Verify that the self-healing infrastructure includes security event correlation combined with automated incident response and stakeholder notification workflows.                                         |   3   |  V   |
...
| 4.15.5 | Verify that hybrid classical-quantum cryptographic systems provide defense-in-depth during the quantum transition period by implementing performance monitoring.                         |   3   | D/V  |
...
|   #    | Description                                                                                                                                                          | Level | Role |
| :----: | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :---: | :--: |
| 4.16.1 | Ensure that sensitive AI models run inside Intel SGX enclaves, AMD SEV-SNP, or ARM TrustZone environments with encrypted memory and attestation verification.        |   3   | D/V  |
...
| 4.16.3 | Ensure that remote attestation confirms enclave integrity by providing cryptographic proof of the execution environment's authenticity before loading AI models.     |   3   | D/V  |
| 4.16.4 | Ensure that confidential AI inference services prevent model extraction by utilizing encrypted computation, sealed model weights, and secure execution.              |   3   | D/V  |
...
| 4.17.3 | Ensure that private set intersection (PSI) protocols facilitate secure data matching in federated AI without revealing individual datasets.                               |   3   | D/V  |
| 4.17.4 | Verify that zero-knowledge machine learning (ZKML) systems enable verifiable AI inferences supported by cryptographic proof of correct computation.                       |   3   | D/V  |
...
|   #    | Description                                                                                                                                        | Level | Role |
| :----: | -------------------------------------------------------------------------------------------------------------------------------------------------- | :---: | :--: |
| 4.18.1 | Ensure that AI inference timing is normalized by using constant-time algorithms and padding to prevent timing-based model extraction attacks.      |   3   | D/V  |
...
|   #    | Description                                                                                                                                                 | Level | Role |
| :----: | ----------------------------------------------------------------------------------------------------------------------------------------------------------- | :---: | :--: |
| 4.19.1 | Verify that neuromorphic chip security includes spike pattern encryption, synaptic weight protection, and hardware-based validation of learning rules.      |   3   | D/V  |
...
| 4.20.4 | Ensure that privacy-preserving key management incorporates distributed key generation, threshold cryptography, and secure key rotation with hardware-backed protection.             |   3   | D/V  |
...
|   #    | Description                                                                                                                  | Level | Role |
| :----: | ---------------------------------------------------------------------------------------------------------------------------- | :---: | :--: |
| 4.15.1 | Verify that cloud storage integration employs end-to-end encryption with key management controlled by the agent.             |   1   | D/V  |
| 4.15.2 | Ensure that the security boundaries of hybrid deployments are clearly defined and that communication channels are encrypted. |   2   | D/V  |
| 4.15.3 | Ensure that access to cloud resources incorporates zero-trust verification with continuous authentication.                   |   2   | D/V  |
