...
| 3.1.2 | Verify that model integrity is validated at deployment time and that signature verification failures prevent the model from loading.                                                   |   1   | D/V  |
...
| 3.1.4 | Ensure that all model artifacts use semantic versioning (MAJOR.MINOR.PATCH) with documented criteria specifying when each version component should be incremented.                     |   2   | D/V  |
| 3.1.5 | Verify that dependency tracking maintains a real-time inventory that enables the rapid identification of all consuming systems.                                                        |   2   |  V   |
...
|   #   | Description                                                                                                                                                                                                      | Level | Role |
| :---: | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :---: | :--: |
| 3.2.1 | Ensure that models undergo automated security testing that includes input validation, output sanitization, and safety evaluations with pre-established organizational pass/fail criteria before deployment.      |   1   | D/V  |
| 3.2.2 | Verify that validation failures automatically prevent model deployment unless explicitly overridden with approval from pre-designated authorized personnel who have provided documented business justifications. |   1   | D/V  |
...
| 3.3.3 | Ensure that deployment processes validate cryptographic signatures and compute integrity checksums before activating the model, and fail the deployment if any mismatch is detected.                                                        |   2   | D/V  |
...
|   #   | Description                                                                                                                                                                                                          | Level | Role |
| :---: | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :---: | :--: |
| 3.4.1 | Verify that all model changes (deployment, configuration, retirement) generate immutable audit records, including a timestamp, an authenticated actor identity, the type of change, and the before and after states. |   1   |  V   |
| 3.4.2 | Ensure that audit log access requires proper authorization and that all access attempts are recorded with the user’s identity and a timestamp.                                                                       |   2   | D/V  |
...
| 3.4.4 | Verify that audit records contain sufficient details (model hashes, configuration snapshots, dependency versions) to allow full reconstruction of the model state for any timestamp within the retention period.     |   2   |  V   |
...
|   #   | Description                                                                                                                                                                                                            | Level | Role |
| :---: | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :---: | :--: |
| 3.5.1 | Verify that the model development, testing, and production environments are physically or logically separated. They should have no shared infrastructure, have distinct access controls, and use isolated data stores. |   1   |  D   |
