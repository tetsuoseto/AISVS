...
| 5.1.3 | Verify that new principals undergo identity proofing aligned with NIST 800-63-3 IAL-2 or equivalent standards before being granted access to production systems.                                                               |   2   |  D   |
| 5.1.4 | Ensure that access reviews are conducted quarterly, incorporating automated detection of dormant accounts, enforcement of credential rotation, and de-provisioning workflows.                                                  |   2   |  V   |
...
| 5.4.2 | Verify that row-level security (RLS) policies and field-level masking are enabled with policy inheritance across all vector databases, search indices, and training datasets.                             |   1   | D/V  |
...
| 5.7.2 | Ensure that high-risk capabilities (file system access, code execution, external API calls, financial transactions) are disabled by default and require explicit authorization with business justifications for activation. |   1   | D/V  |
| 5.7.3 | Verify that capability tokens are tied to user sessions, include cryptographic integrity protection, and ensure they cannot be saved or reused in offline scenarios.                                                        |   2   |  D   |
| 5.7.4 | Ensure that agent-initiated actions are subject to secondary authorization via the ABAC policy engine, including full context evaluation and audit logging.                                                                 |   2   |  V   |
