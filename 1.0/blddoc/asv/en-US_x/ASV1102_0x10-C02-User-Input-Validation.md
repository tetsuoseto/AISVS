...
| :---: | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :---: | :--: |
| 2.1.1 | Verify that user inputs are checked against a continuously updated library of known prompt injection patterns (jailbreak keywords, "ignore previous," role-play chains, indirect HTML/URL attacks).                             |   1   | D/V  |
...
| 2.1.4 | Ensure that prompts coming from third-party sources (such as web pages, PDFs, and emails) are sanitized within an isolated parsing environment before being combined into the main prompt.                                      |   2   |  D   |
...
| 2.3.2 | Ensure that inputs exceeding the maximum token or byte limits are rejected with a safe error message and never silently truncated.                                                               |   1   | D/V  |
...
| 2.4.4 | Verify that screening complies with user-specific policies (such as age and regional legal restrictions) using attribute-based rules that are resolved at the time of the request.                  |   2   |  D   |
...
|   #   | Description                                                                                                             | Level | Role |
| :---: | ----------------------------------------------------------------------------------------------------------------------- | :---: | :--: |
| 2.5.1 | Verify that per-user, per-IP, and per-API-key rate limits are enforced on all input endpoints.                          |   1   | D/V  |
...
| 2.5.4 | Ensure that abuse prevention logs are retained and reviewed for emerging attack patterns.                               |   3   |  V   |
...
| 2.7.3 | Ensure that anomalous or untrusted input sources are flagged and subjected to increased scrutiny or blocking.          |   2   | D/V  |
...
| 2.8.2 | Ensure that threat detection systems maintain separate pattern libraries for different threat categories (prompt injection, harmful content, sensitive data, system commands).             |   1   | D/V  |
...
| 2.9.2 | Verify that multi-modal inputs are processed within isolated sandboxes that have defined resource limits (memory, CPU, processing time) specific to each modality type and documented in the security policies.                       |   2   | D/V  |
