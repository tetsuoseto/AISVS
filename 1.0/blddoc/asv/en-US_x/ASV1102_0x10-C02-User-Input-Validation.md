...
| 2.1.3 | Ensure that adversarial evaluation tests (e.g., Red Team "many-shot" prompts) are conducted before every model or prompt-template release, with success-rate thresholds and automated blockers in place to prevent regressions. |   2   | D/V  |
| 2.1.4 | Ensure that prompts coming from third-party sources (web pages, PDFs, emails) are sanitized within a separate parsing context before being combined into the main prompt.                                                       |   2   |  D   |
| 2.1.5 | Ensure that all prompt-filter rule updates, classifier model versions, and block-list changes are version-controlled and auditable.                                                                                             |   3   | D/V  |
...
|   #   | Description                                                                                                                                                                                | Level | Role |
| :---: | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | :---: | :--: |
| 2.2.1 | Verify that basic input normalization steps (Unicode NFC, homoglyph mapping, whitespace trimming) occur before tokenization.                                                               |   1   |  D   |
...
| 2.2.4 | Ensure that suspected adversarial inputs are quarantined and logged with complete payloads (after PII redaction).                                                                          |   2   |  V   |
| 2.2.5 | Ensure that robustness metrics (success rate of known attack suites) are monitored over time and that any regressions trigger a release blocker.                                           |   3   | D/V  |
...
|   #   | Description                                                                                                                                                                                      | Level | Role |
| :---: | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | :---: | :--: |
| 2.3.1 | Verify that every API or function call endpoint defines an explicit input schema (JSON Schema, Protobuf, or a multimodal equivalent) and that inputs are validated before assembling the prompt. |   1   |  D   |
...
| 2.3.3 | Ensure that type checks (such as numeric ranges, enum values, and MIME types for images/audio) are enforced on the server side, not just in the client code.                                     |   2   | D/V  |
...
| 2.4.2 | Ensure that inputs violating policies receive standardized refusals or safe completions to prevent their propagation to downstream LLM calls.                                                |   1   | D/V  |
...
| 2.4.4 | Verify that screening complies with user-specific policies (age, regional legal constraints) through attribute-based rules evaluated at request time.                                        |   2   |  D   |
...
|   #   | Description                                                                                                              | Level | Role |
| :---: | ------------------------------------------------------------------------------------------------------------------------ | :---: | :--: |
| 2.6.1 | Ensure that all non-text inputs (images, audio, files) are validated for their type, size, and format before processing. |   1   |  D   |
...
|   #   | Description                                                                                                                                                                             | Level | Role |
| :---: | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :---: | :--: |
| 2.8.1 | Ensure that threat detection patterns are compiled into optimized regex engines to achieve high-performance real-time filtering with minimal latency impact.                            |   1   | D/V  |
